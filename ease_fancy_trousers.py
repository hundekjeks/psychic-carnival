...
from numba import njit, prange, float64, int64, complex128, void

# ====================================================================
# 1. CORE SINGULARITY PATCHING ENGINE (STRICT INT64 JIT TYPING)
# ====================================================================
@njit(
    void(
...
def _numba_patch_singularities(
    ...
    for i in range(num_elements):
        ...
        if abs_wi < 1e-8 or abs_L < 1e-7:
            ...
            I1 = -1j * wi / (np.pi**2)
            I2 = complex(2.0 / np.pi, 0.0)
            out[i] = L * gp * ((c0 * I0) - (c1 * I1) + (c2 * I2))

        # --- CASE 2: Positive Resonance Pole (w ≈ +pi) ---
        elif np.abs(wi - np.pi) < 1e-7:
            dw = wi - np.pi
            sinc_dw = 1.0 if dw == 0.0 else np.sin(dw) / dw
            half_phase = np.cos(-0.5 * dw) + 1j * np.sin(-0.5 * dw)
            I1 = 0.5 * sinc_dw * half_phase
            I2 = -0.5j * sinc_dw * half_phase
            I0 = (np.sin(wi) / wi) * (
                np.cos(-0.5 * wi) + 1j * np.sin(-0.5 * wi)
            )
            out[i] = L * gp * ((c0 * I0) - (c1 * I1) + (c2 * I2))

        # --- CASE 3: Negative Resonance Pole (w ≈ -pi) ---
        elif np.abs(wi + np.pi) < 1e-7:
            dw = wi + np.pi
            sinc_dw = 1.0 if dw == 0.0 else np.sin(dw) / dw
            half_phase = np.cos(-0.5 * dw) + 1j * np.sin(-0.5 * dw)
...
def _numba_integrate_cpu_engine(
    ...
    """
    num_elements = n_arr.shape
    out = np.empty(num_elements, dtype=np.complex128)

    # --- SCALAR ACCURACY GUARDS ---
    if (np.isnan(x0) or np.isnan(x1) or np.isnan(y0) or np.isnan(y1) or
        np.isnan(k) or np.isinf(x0) or np.isinf(x1) or np.isinf(y0) or
        np.isinf(y1) or np.isinf(k)):
        out.fill(complex(np.nan, np.nan))
        return out
    ...
    two_pi_x0 = 2.0 * np.pi * x0

    # Statically typed temporary execution arrays
    w = np.empty(num_elements, dtype=np.float64)
    global_phase = np.empty(num_elements, dtype=np.complex128)
    needs_patching = np.zeros(num_elements, dtype=np.bool_)
    ...
    for i in prange(num_elements):
        ...
        if (np.abs(wi) < 1e-8 or abs_L < 1e-7 or 
            ...
            I2 = -np.pi * (exp_w + 1.0) / denom

            out[i] = L * gp * ((c0 * I0) - (c1 * I1) + (c2 * I2))

    # --- STRIDE 2: Singular Matrix Patches ---
    if np.any(needs_patching):
...
# ====================================================================
# 3. PYTHON WRAPPER INTERFACE & SPECIALIZATIONS (EXPLICIT TYPING)
# ====================================================================
def integrate_h_exponential_final(
    ...
    n_arr = np.ascontiguousarray(n, dtype=np.int64)
...
def integrate_h_exponential_n0(
    x0: float, x1: float, y0: float, y1: float, k: float
) -> float:
    """Computes exact analytical integral for n = 0 as a real float."""
    n_arr = np.array([0], dtype=np.int64)
...
def integrate_h_exponential_n1(
    ...
    n_arr = np.array([1], dtype=np.int64)
    res = _numba_integrate_cpu_engine(
        n_arr, float(x0), float(x1), float(y0), float(y1), float(k)
    )
